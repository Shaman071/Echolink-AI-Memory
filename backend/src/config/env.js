...
const envVarsSchema = Joi.object({
  ...
  JWT_EXPIRE: Joi.string().default('7d').description('JWT access token expiration time'),
  JWT_REFRESH_EXPIRE: Joi.string().default('30d').description('JWT refresh token expiration time'),
  JWT_COOKIE_EXPIRE: Joi.number().default(30).description('JWT cookie expire days'),
  MAX_FILE_SIZE: Joi.number().default(10485760).description('Max file size in bytes (10MB)'),
  ...
  OPENAI_API_KEY: Joi.string().allow('').optional().description('OpenAI API key'),
...
module.exports = {
  ...
  jwt: {
    ...
    refreshExpire: envVars.JWT_REFRESH_EXPIRE || '30d',
    cookieExpire: envVars.JWT_COOKIE_EXPIRE,
    expireNumDays: 7, // Access token expiry in days
    refreshExpireNumDays: 30, // Refresh token expiry in days
  },
  upload: {
